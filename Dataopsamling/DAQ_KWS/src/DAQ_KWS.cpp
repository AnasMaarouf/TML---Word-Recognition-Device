...
const unsigned long MAX_RECORDING_LENGTH_MS = 30000; // Changed to 1 s pr sample

// This is the IP Address and port that the server.js node server is running on.
IPAddress serverAddr = IPAddress(172,20,10,4); // **UPDATE THIS** UPDATED TO RUN ON MY "IPhone" HOTSPOT
int serverPort = 7123;
...
void loop() {
	...
	case STATE_CONNECT:
		...
		if (client.connect(serverAddr, serverPort)) {
			...
			// Delay of 300 ms, for at kompensere for knaptryk på "MODE"
			delay(300);
			//
