...
// ----------------------- EGEN TILFØJELSE - TILFØJET SÅ AT VI KAN LAVE LABEL RECORDING
const label = argv.label || "recording";
// -----------------------
...
// --------------- EGEN TILFØJELSE - BRUG OGSÅ LABEL, SOM BLEV TILFØJET AF OS SELV
function formatName(num) {
	var s = num.toString();
	while(s.length < 3) {
		s = '0' + s;
...
// -------------- EGEN TILFØJELSE
function getUniqueOutputPath() {
	for(var ii = lastNum + 1; ii < 999; ii++) {
		var outPath = path.join(outputDir, formatName(ii));
		try {
			fs.statSync(outPath);
		}
