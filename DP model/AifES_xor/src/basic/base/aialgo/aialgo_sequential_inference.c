...
uint32_t aialgo_sizeof_inference_memory(aimodel_t *model)
{
	...
	for (i = 0; i < model->layer_count; i++)
	{
		...
		if (!AILAYER_SETTINGS_IS(layer_ptr->settings, 0b1, AILAYER_SETTINGS_KEEP_INPUT_BUFFER_FOR_RESULT))
		{
			...
			if (temp_memory > max_memory)
				max_memory = temp_memory;
		}
		...
		if (layer_ptr->sizeof_fwdmem != 0)
		{
			...
			if (temp_memory > max_memory2)
				max_memory2 = temp_memory;
		}
...
uint8_t aialgo_schedule_inference_memory(aimodel_t *model, void *memory_ptr, uint32_t memory_size)
{
	...
	for (i = 0; i < model->layer_count; i++)
	{
		...
		if (layer_ptr->sizeof_fwdmem != 0)
		{
			if (layer_ptr->sizeof_fwdmem(layer_ptr) > max_memory)
				max_memory = layer_ptr->sizeof_fwdmem(layer_ptr);
		}
	...
	for (i = 0; i < model->layer_count; i++)
	{
		...
		if (layer_ptr->output_layer == 0 || !AILAYER_SETTINGS_IS(layer_ptr->output_layer->settings, 0b1, AILAYER_SETTINGS_KEEP_INPUT_BUFFER_FOR_RESULT))
		{
			idx++;
		}
		layer_ptr = layer_ptr->output_layer;
...
{
	AIPRINT("____model layers______");
	if (model == NULL || model->input_layer == NULL)
	{
		AIPRINT("!!!!!!model is NULL!!!!!!!");
		return; // Early exit if model or input layer is invalid
	}
	int i;
	ailayer_t *layer_ptr = model->input_layer;
